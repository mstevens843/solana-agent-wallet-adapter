...
| Jupiter | First-class | Swap API v2 order preview, swap quote, Token API V2 metadata/tags/categories/recent, Price API V3 prices, token risk evidence, beta Prediction events/markets/orderbook/wallet reads (opt-in) | Prepared swap, direct wallet swap | Token/Price reads are evidence only and not oracle guarantees. Prediction is beta, read-only, and disabled by default until `connectors.jupiter.prediction.enabled=true`. Trigger, Recurring, and Perps remain roadmap surfaces. |
