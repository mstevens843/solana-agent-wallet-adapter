...
| Jupiter | First-class | Swap API v2 order preview, swap quote, Token API V2 metadata/tags/categories/recent, Price API V3 prices, token risk evidence, Lend Earn/Borrow reads, beta Prediction events/markets/orderbook/wallet reads (opt-in), Trigger V2 auth/vault/orders (opt-in), Recurring native DCA orders/quote (opt-in), Perps status (read-only research, account snapshots return unsupported_method) | Prepared swap, direct wallet swap, Lend Earn/Borrow prepared actions, Trigger V2 prepared actions (opt-in), Recurring native DCA setup/cancel/deprecated price management (opt-in) | Token/Price reads are evidence only and not oracle guarantees. Prediction is beta and read-only until `connectors.jupiter.prediction.enabled=true`. Trigger is disabled by default until `connectors.jupiter.trigger.enabled=true`, requires volatile JWT auth, and uses a Privy custody vault plus Jupiter automation. Recurring is disabled by default until `connectors.jupiter.recurring.enabled=true`; future DCA fills execute through Jupiter automation, not Agentic approval-per-run. Perps is read-only research; the official API is work in progress, so account decoding stays gated and all Perps/JLP writes are denied. |
