...
| Jupiter | First-class | Swap API v2 order preview, swap quote, Token API V2 metadata/tags/categories/recent, Price API V3 prices, token risk evidence, Lend Earn/Borrow reads, beta Prediction events/markets/orderbook/wallet reads (opt-in), Trigger V2 auth/vault/orders (opt-in), Perps status (read-only research, account snapshots return unsupported_method) | Prepared swap, direct wallet swap, Lend Earn/Borrow prepared actions, Trigger V2 prepared actions (opt-in) | Token/Price reads are evidence only and not oracle guarantees. Prediction is beta and read-only until `connectors.jupiter.prediction.enabled=true`. Trigger is disabled by default until `connectors.jupiter.trigger.enabled=true`, requires volatile JWT auth, and uses a Privy custody vault plus Jupiter automation. Perps is read-only research; the official API is work in progress, so account decoding stays gated and all Perps/JLP writes are denied. Recurring remains roadmap. |
