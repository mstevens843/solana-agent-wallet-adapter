...
- **CVE-2025-71329 / GHSA-5p2g-fcmc-qvqq** — `image-size <=2.0.2`: JXL and HEIF parser infinite-loop DoS on crafted image buffers. Reaches the tree through `@walletconnect/sign-client`'s React Native / Metro dependency chain (`@react-native-async-storage/async-storage` -> Metro -> `image-size`). No patched version is published. The application does not expose server-side or bridge-side image probing with `image-size`, and this dependency path is tooling/runtime compatibility for React Native packages rather than an upload parser. Status: tracked and ignored in audit/OSV until upstream publishes a fix.
- **CVE-2025-71330 / GHSA-w3rx-r6r6-pgpr** — `image-size <=2.0.2`: ICNS parser infinite-loop DoS on crafted image buffers. Same reachability as CVE-2025-71329: present through React Native / Metro transitive dependencies, with no untrusted image parsing path in this repository and no upstream patch yet. Status: tracked and ignored in audit/OSV until upstream publishes a fix.
