...
object WalletDetector {
    ...
    val KNOWN_WALLETS: List<KnownWallet> = listOf(
        ...
        KnownWallet("Backpack", "app.backpack.mobile", "https://play.google.com/store/apps/details?id=app.backpack.mobile", WalletRegistry.BACKPACK),
    ...
    fun detectInstalledWallets(context: Context): JSONArray {
        ...
        for (wallet in KNOWN_WALLETS) {
            val info = try {
                ...
                pm.getPackageInfo(wallet.packageName, 0)
                ...
                AgentMwaLog.warn(
                    ...
                    mapOf("package" to wallet.packageName, "class" to err.javaClass.simpleName, "message" to err.message),
                )
                null
            }
            results.put(
                JSONObject()
                    ...
                    .put("packageName", wallet.packageName)
