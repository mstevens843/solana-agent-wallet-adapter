// Lifted from apps/ios-native/Sources/AgenticNativeIOS/AgenticAuthCache.swift
// (Phase 0.5). AgenticIOSNativeLog references rewritten to AgenticIOSLog.
import Foundation
import Security
...
final class AgenticAuthCache {
    ...
    func clear(publicKey: String) {
        ...
        writeRoot(root)
        AgenticIOSLog.info("AgenticAuthCache", "clear", "DONE", "authorization cleared", ["pubkey": short(publicKey)])
    }

    func clearAll() {
        writeRoot(AgenticAuthCacheRoot())
        AgenticIOSLog.info("AgenticAuthCache", "clearAll", "DONE", "all authorizations cleared")
    }
    ...
    private func writeRoot(_ root: AgenticAuthCacheRoot) {
        do {
            ...
            AgenticIOSLog.fail("AgenticAuthCache", "writeRoot", "FAIL", "cache write failed", ["error": error.localizedDescription])
        }
    }
