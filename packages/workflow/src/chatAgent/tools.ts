...
export function chatToolsAnthropic(): Array<Record<string, unknown>> {
  return [
    ...
    },
    {
      ...
      description: "Get live, pre-formatted facts for a wallet protocol connector action. Jupiter (connectorId 'jupiter', default): lend/borrow/limit/dca/perps/prediction/swap. Raydium / Orca / Meteora: liquidity. Kamino / Lulo: lend. Jito / Marinade: stake. Drift: vault. Wormhole: bridge. Pyth: oracle. MarginFi / Project 0 / Save: markets/lend/borrow (Project 0 also strategies). Magic Eden / Tensor: marketplace/wallet NFT facts. Sanctum: stake/liquidity/swap. Realms: governance. Squads: governance/treasury. Phoenix: markets/perps. Use this for protocol positions, connector rates, NFT marketplace floors/listings, governance/multisig/proposal facts, staking/LST/bridge/oracle facts, or perps positions/markets. For plain token spot prices/safety use get_token_price / get_token_safety instead.",
      input_schema: {
        type: 'object',
        properties: {
          connectorId: { type: 'string', description: "Connector id: jupiter (default) | raydium | orca | meteora | kamino | jito | marinade | drift | lulo | wormhole | pyth | marginfi | project0 | save | magiceden | tensor | sanctum | realms | squads | phoenix" },
          action: { type: 'string', description: 'jupiter: lend|borrow|limit|dca|perps|prediction|swap. raydium/orca/meteora: liquidity. kamino/lulo: lend. jito/marinade: stake. drift: vault. wormhole: bridge. pyth: oracle. marginfi/project0/save: markets|lend|borrow. project0: strategies. magiceden/tensor: marketplace|wallet. sanctum: stake|liquidity|swap. realms: governance. squads: governance|treasury. phoenix: markets|perps' },
          ...
          token: { type: 'string', description: 'Optional protocol token/symbol for market reads (e.g. SOL, USDC, SOL-PERP)' },
          bankAddress: { type: 'string', description: 'Optional MarginFi / Project 0 bank address' },
          bankMint: { type: 'string', description: 'Optional MarginFi / Project 0 bank mint' },
          marginfiAccount: { type: 'string', description: 'Optional MarginFi account address for a single-account read' },
          project0Account: { type: 'string', description: 'Optional Project 0 account address for a single-account read' },
          collectionId: { type: 'string', description: 'Optional Tensor / Magic Eden collection id' },
          collectionSymbol: { type: 'string', description: 'Optional Magic Eden collection symbol' },
          mintAddress: { type: 'string', description: 'Optional NFT mint address for Magic Eden / Tensor NFT detail' },
          assetId: { type: 'string', description: 'Optional compressed NFT asset id for Tensor NFT detail' },
          lstMint: { type: 'string', description: 'Optional Sanctum LST mint' },
          inputMint: { type: 'string', description: 'Optional connector quote input mint' },
          outputMint: { type: 'string', description: 'Optional connector quote output mint' },
          realmAddress: { type: 'string', description: 'Optional Realms realm address' },
          governanceAddress: { type: 'string', description: 'Optional Realms governance address' },
          proposalAddress: { type: 'string', description: 'Optional Realms or Squads proposal address' },
          multisigAddress: { type: 'string', description: 'Optional Squads multisig address' },
          vaultIndex: { type: 'number', description: 'Optional Squads vault index for treasury reads' },
          transactionIndex: { type: 'number', description: 'Optional Squads transaction index for proposal reads' },
          subAccountId: { type: 'number', description: 'Optional Drift/Phoenix sub-account or trader PDA index' },
          includeListings: { type: 'boolean', description: 'Optional NFT marketplace flag to include listings' },
          includeBids: { type: 'boolean', description: 'Optional NFT marketplace flag to include bids' },
          listedOnly: { type: 'boolean', description: 'Optional NFT wallet flag to show only listed NFTs' },
