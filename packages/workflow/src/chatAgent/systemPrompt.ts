...
export function chatAgenticSystemPrompt(request: ChatPromptContext): string {
  ...
  return [
    ...
    'CONNECTORS: For DeFi connector actions (Jupiter lend/earn positions, borrow positions & health, limit/TP-SL orders, DCA/recurring orders, perps status, prediction markets) call get_connector_facts with connectorId (default jupiter) and action (lend|borrow|limit|dca|perps|prediction). connectorContext lists the available actions and the selected connector card - use it to explain what a connector can do; for live positions/orders/health you MUST call get_connector_facts first and never invent them. If a result has "unavailable":true or names a disabled product, say it is unavailable or needs to be enabled.',
