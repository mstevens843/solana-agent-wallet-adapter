...
For downloaded CLI or desktop builds, run `solana-agent-wallet setup` or use the desktop Transaction Setup panel. This writes the local runtime `.env` with `SOLANA_RPC_URL`, `HELIUS_RPC_URL`, `JUPITER_API_KEY`, `JUP_API_KEY`, `JUPITER_SWAP_BASE_URL`, legacy `JUP_ULTRA_BASE`, and `JUPITER_API_URL` so sends and Swap API v2 swaps can execute after wallet approval. Optional Jupiter Pro product base URLs and CoinGecko keys in `.env.example` expand Ask/Check review evidence; they do not bypass Needs Approval signing.
